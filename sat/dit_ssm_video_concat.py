...
from mamba_ssm.models.mixer_seq_simple import create_block as create_mamba_block, Block as SsmMixerBlock
...
from ttt.ttt_seq_simple import create_block as create_ttt_block
...
def create_block(
    ...
    assert ssm_cfg is not None, "SSM not supported"
    ssm_layer = ssm_cfg["layer"]
    if "Mamba" in ssm_layer:
        return create_mamba_block(
            d_model=d_model,
    ...
    elif "TTT" in ssm_layer:
        return create_ttt_block(
            d_model=d_model,
            d_intermediate=d_intermediate,
            ssm_cfg=ssm_cfg,
            norm_epsilon=norm_epsilon,
        ...
        raise ValueError(f"Invalid ssm_layer: {ssm_layer}, only support Mamba and TTT")
...
class ScanBlock(nn.Module):
    def __init__(self, block, in_features, out_features, direction="forward"):
        super().__init__()
        assert isinstance(block, SsmMixerBlock) or block is None
        self.block = block
    ...
    def forward(self, x, direction_start_idx, block=None):
        if block is None:
            block = self.block
        else:
...
class SsmAttentionLayerMixin(BaseMixin):
    def __init__(
        self,
        num_layers,
        hidden_size,
        ...
        ssm_order="post",
        repeat_attn_steps=0,
        ssm_gating_func="tanh",
        ssm_gating_alpha_init=0.,
        parallel_scan=False,
        layer_type="TTTLinearTriton",
        ssm_share_scan=False,
    ):
        super().__init__()
        self.num_layers = num_layers
        ...
        self.ssm_scan1_list = nn.ModuleList([
            ScanBlock(
                create_block(
                    d_model=ssm_hidden_size if ssm_hidden_size > 0 else hidden_size,
                    d_intermediate=0,
                    ssm_cfg=dict(
                        layer=layer_type,
                        headdim=head_dim,
                    ...
                    fused_add_norm=fused_add_norm
                ) if not ssm_share_scan else None,
                in_features=hidden_size,
                out_features=ssm_hidden_size,
                direction="forward",
            ) for _ in range(num_layers)
        ])
        self.scan1_gating_list = nn.ModuleList([GatingModule(hidden_size, ssm_gating_func, ssm_gating_alpha_init) for _ in range(num_layers)])
        self.ssm_scan2_list = nn.ModuleList([
            ScanBlock(
                create_block(
                    d_model=ssm_hidden_size if ssm_hidden_size > 0 else hidden_size,
                    d_intermediate=0,
        ...
        self.scan2_gating_list = nn.ModuleList([GatingModule(hidden_size, ssm_gating_func, ssm_gating_alpha_init) for _ in range(num_layers)])
        if ssm_share_scan:
            self.ssm_share_scan_block = nn.ModuleList([create_block(
                    d_model=ssm_hidden_size if ssm_hidden_size > 0 else hidden_size,
                    d_intermediate=0,
                    ssm_cfg=dict(
                        layer=layer_type,
            ...
            self.ssm_share_scan_block = [None] * num_layers
        self.temporal_length = compressed_num_frames
        self.prefix_temporal_length = prefix_temporal_length
        self.attn_length = attn_length
        assert ssm_order in ["pre", "post"], "ssm_order must be pre or post"
        self.ssm_order = ssm_order
        self.repeat_attn_steps = repeat_attn_steps
        self.parallel_scan = parallel_scan

    def _ssm_forward(self, hidden_states, **kwargs):
        layer_idx = kwargs["layer_id"]
        text_length = kwargs["text_length"]

        if self.parallel_scan:
            # scan1 forward
            scan1_hidden_states = self.ssm_scan1_list[layer_idx](hidden_states, text_length, block=self.ssm_share_scan_block[layer_idx])
            # scan2 forward
            scan2_hidden_states = self.ssm_scan2_list[layer_idx](hidden_states, text_length, block=self.ssm_share_scan_block[layer_idx])
            
            # parallel add
            ssm_output = hidden_states + self.scan1_gating_list[layer_idx](scan1_hidden_states) + self.scan2_gating_list[layer_idx](scan2_hidden_states)
        else:
            # scan1 forward
            scan1_hidden_states = self.ssm_scan1_list[layer_idx](hidden_states, text_length, block=self.ssm_share_scan_block[layer_idx])
            ...
            # scan2 forward
            scan2_hidden_states = self.ssm_scan2_list[layer_idx](scan1_output, text_length, block=self.ssm_share_scan_block[layer_idx])
            ssm_output = scan1_output + self.scan2_gating_list[layer_idx](scan2_hidden_states)
    ...
    def attention_forward(self, hidden_states, mask, **kwargs):
        if self.ssm_order == "post":
            attn_output = self._attention_forward(hidden_states, mask, **kwargs)
            ssm_output = self._ssm_forward(attn_output, **kwargs)
            return ssm_output
        elif self.ssm_order == "pre":
            ssm_output = self._ssm_forward(hidden_states, **kwargs)
            attn_output = self._attention_forward(ssm_output, mask, **kwargs)
            return attn_output
        else:
            raise ValueError(f"Invalid ssm_order: {self.ssm_order}")
...
class DiffusionTransformer(BaseModel):
    ...
    def _build_modules(self, module_configs):
        ...
        ssm_attn_config = module_configs["ssm_attn_config"]
        self.add_mixin("ssm_attn", instantiate_from_config(
            ssm_attn_config, 
            num_layers=self.num_layers, 
            hidden_size=self.hidden_size, 

...
def save_checkpoint(iteration, model_engine, optimizer, lr_scheduler, args):
    save_dir = args.ckpt_dir

    sd = {}
    sd['iteration'] = args.iteration
    # rng states.
    ...
    tag = str(iteration)
    print_rank0(f"Saving checkpoint at iteration {iteration} to {save_dir}")
    model_engine.save_checkpoint(save_dir, tag, client_state=sd, exclude_frozen_parameters=True)
    if mpu.get_data_parallel_rank() == 0:
        delete_old_checkpoints(save_dir, max_checkpoints=3)
    
    if optimizer is not None and optimizer.optimizer.__class__.__name__ ==  "FusedEmaAdam" :
        update_ema_parameters_to_model(optimizer)
        if mpu.get_data_parallel_rank() == 0:
            # model_engine.save_checkpoint(save_dir, tag+'-ema', client_state=sd, save_latest=False)
            print_rank0('Saving Ema Model...')
            save_ds_checkpoint(iteration, model_engine, lr_scheduler, args, use_ema=True, save_latest=False)
            delete_old_checkpoints(save_dir, max_checkpoints=3, use_ema=True)
        restore_ema_parameters_back(optimizer)


def save_ds_checkpoint(iteration, model, lr_scheduler, args, use_ema = False, save_latest=True):
    ...
    if not args.no_save_rng:
        ...
        sd['cuda_rng_state'] = torch.cuda.get_rng_state()
    if not use_ema:
        save_ds_checkpoint_no_optim(model, save_dir, str(iteration), client_state=sd, save_latest=save_latest)
    else:
        save_ds_checkpoint_no_optim(model, save_dir, str(iteration)+'-ema', client_state=sd, save_latest=save_latest)
...
def resume_checkpoint(model, resume_dir, strict=True):
    """resume a model checkpoint."""

    _, client_sd = model.load_checkpoint(resume_dir, load_module_strict=strict)

    return client_sd


def delete_old_checkpoints(save_dir, max_checkpoints=5, use_ema=False):
    # List all checkpoint directories
    checkpoints = glob.glob(os.path.join(save_dir, '*'))
    checkpoints = [ckpt for ckpt in checkpoints if os.path.isdir(ckpt)]

    # Extract iteration numbers and sort by them
    def extract_iteration(ckpt):
        match = re.search(r'(\d+)(-ema)?$', ckpt)
        if match:
            iteration = int(match.group(1))
            if match.group(2) == '-ema' or match.group(2) is None:
                return iteration
        return -1
    
    # filter out -1 iteration
    checkpoints = [ckpt for ckpt in checkpoints if extract_iteration(ckpt) > 0]
    
    if use_ema:
        checkpoints = [ckpt for ckpt in checkpoints if ckpt.endswith('-ema')]
    else:
        checkpoints = [ckpt for ckpt in checkpoints if not ckpt.endswith('-ema')]
    ...
    # Delete oldest checkpoints if there are more than max_checkpoints
    while len(checkpoints) > max_checkpoints:
        # os.rmdir(checkpoints.pop(0))
        first_ckpt = checkpoints.pop(0)
        print_rank0(f"Deleting old checkpoint {first_ckpt}")
        shutil.rmtree(first_ckpt)
...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None):
    ...
    for key in missing_keys:
        if any(key.startswith(ignore_key) for ignore_key in ['first_stage_model.', 'conditioner.']):
            continue
    ...
    if len(missing_keys) > 0:
        if args.mode == 'inference':
            ...
            print_rank0(f'Reinit mixins: {mixin_names}')

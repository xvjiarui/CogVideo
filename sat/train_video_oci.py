...
from sat import mpu
from sat.model import transformer
# NOTE(xvjiarui): hack mpu.checkpoint to use deepspeed non_reentrant_checkpoint
from deepspeed.runtime.activation_checkpointing.checkpointing import non_reentrant_checkpoint
mpu.checkpoint = non_reentrant_checkpoint
transformer.checkpoint = non_reentrant_checkpoint
del transformer


from sat.helpers import logger
...
def parse_args(input_args=None):
    ...
    if not args.experiment_name:
        base_names = [os.path.basename(os.path.splitext(base)[0]) for base in args.base]
        args.experiment_name = '_'.join(base_names)
    if args.tag:
    ...
    if args.save:
        args.summary_dir = os.path.join(args.save, args.experiment_name)
        args.save = os.path.join(args.save, args.experiment_name)
        args.ckpt_dir = os.path.join(args.save, "checkpoints")
...
def configure_logging(output, distributed_rank):
...
def main(args):
    ...
    configure_logging(log_dir, torch.distributed.get_rank())

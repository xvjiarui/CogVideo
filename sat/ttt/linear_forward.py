...
def ttt_linear_scan_forward(
    ...
    accum_dtype = W1_init_ptr.type.element_ty
    ...
    for i in range(NC):
        ...
        W1_init = W1_init - tl.dot(tl.trans(last_eta_mini_batch * XK_mini_batch).to(comp_dtype), grad_l_wrt_Z1).to(
            accum_dtype
        )
        b1_init = b1_init - tl.sum(last_eta_mini_batch * grad_l_wrt_Z1, axis=0)[None, :].to(accum_dtype)
